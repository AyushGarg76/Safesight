...
VIDEO_PATH = os.path.join('..', '..', 'test_video5.mp4')
...
def main():
    ...
    # Resolve video path
    video_input = VIDEO_PATH if os.path.exists(VIDEO_PATH) else 'test_video5.mp4'
    ...
    frame_idx   = 0
    frames_buf  = []   # raw BGR frames
    tensors_buf = []   # preprocessed tensors
    ...
    while cap.isOpened():
        ...
        if needs_infer:
            ...
            # When batch is full (or last frame), run inference
            if len(tensors_buf) == BATCH_SIZE or not cap.isOpened():
                outputs = run_batch(model, tensors_buf, use_amp)

                for i, (buf_frame, output) in enumerate(zip(frames_buf, outputs)):
                    ...
                    if SHOW_PREVIEW:
                        cv2.imshow('Helmet Detection V4', buf_frame)
            ...
            # Reuse last detections — no inference cost
            frame = draw_detections(frame, last_boxes, last_scores,
                                    last_labels, scale_x, scale_y)
            out.write(frame)

            if SHOW_PREVIEW:
                cv2.imshow('Helmet Detection V4', frame)
                if cv2.waitKey(1) & 0xFF == ord('q'):
                    break
    ...
    if tensors_buf:
        outputs = run_batch(model, tensors_buf, use_amp)
        for buf_frame, output in zip(frames_buf, outputs):
            last_boxes  = output['boxes'].cpu().numpy()
            last_scores = output['scores'].cpu().numpy()
            last_labels = output['labels'].cpu().numpy()
            buf_frame = draw_detections(buf_frame, last_boxes, last_scores,
